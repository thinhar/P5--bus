...
Now that the object-oriented analysis and design has been finished, and the functions have been found, we will make a risk vs value evaluation order to figure out which functionalities should be the main focus of the project.%at the 
%\info{This part is new.}function list \ref{functionlist} to find the possible functionalities which should be created in order to make the automated bus system. 
%However, making them all would take too much time, and some of them might not be worth it to create, so to help determine what functionality should be the focus point an evaluation is made using a risk vs value model to help prioritising.
